  ...
  class CSF_Field_radio extends CSF_Fields {
    ...
    public function render() {
      ...
      if ( isset( $this->field['options'] ) ) {
        ...
        if ( is_array( $options ) && ! empty( $options ) ) {
          ...
          foreach ( $options as $option_key => $option_value ) {
            ...
            if ( is_array( $option_value ) && ! empty( $option_value ) ) {
              ...
              echo '<li><label><input type="radio" name="'. esc_attr( $this->field_name() ) .'" value="'. esc_attr( $option_key ) .'"'. $this->field_attributes() . esc_attr( $checked ) .'/> '. ( $option_value ) .'</label></li>';
        ...
        echo '<label><input type="radio" name="'. esc_attr( $this->field_name() ) .'" value="1"'. $this->field_attributes() . esc_attr( checked( $this->value, 1, false ) ) .'/> '. ( $label ) .'</label>';
      }
